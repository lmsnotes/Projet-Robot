...
volatile uint16_t Vbatt; //Valeur de tension de l'alimentation
...
int main(void)
{
  ...
  {
  	  	while(start){
  	  		if(Tbatt > 50) //Vérification batterie du robot
  	  		  	  {
  	  		  		  HAL_ADC_Start_IT(&hadc1);
  	  		  	  	  if(Vbatt < seuil_batt) //Robot déchargé
  	  		  	  	  {
  	  		  	  		HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, SET); // LED2 on car robot déchargé
  	  		  	  	  }
  	  		  	  	  else //Robot chargé
  	  		  	  	  {
  	  		  	  		HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, RESET); //Eteind LED2
  	  		  	  	  }
  	  		  	  	  Tbatt = 0; //Remise a zero du compteur
  	  		  	  	  }


  	  	    if(dir == AVANCE){
  	  	            if(avance == 0) //A l'arrêt
  	  	            avancer();
  		  ...
  		  if(dir == DROITE)
  		  {
  			  if(avance == 1 && !tourner) //En train d'avancer et n'a pas encore prévu de tourner
  			  {
  				  time_tourne = 0;
  				  tourner = 1;
  			  }
  	        ...
  	        if ((time_tourne > 1) && (time_tourne <= 250) && (Buffer_RX[0] != DROITE) && (tourner == 1)){ //Appui court sur droite
  	    				  HAL_GPIO_WritePin(GPIOC, DIR1_Pin, GPIO_PIN_RESET);
  	    				  tourne_court = 1; //doit tourner de 45 degrés
  	    				  dir = 0;
  	    		  }
  	        if(time_tourne > 250 && tourner == 1){ //On a appuyé sur tourner et l'appui est long
  	    				  while(Buffer_RX[0] == DROITE)//tant qu'on appuie on continue de tourner
  	    				  {
  	    					  HAL_GPIO_WritePin(GPIOC, DIR1_Pin, GPIO_PIN_RESET);
  	    				  }
  	    				  tourner = 0;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	UNUSED(htim);
	Tbatt++;
	if(tourner) time_tourne++;
	if((tourne_court == 1) && (time_tourne >= 224)){
			HAL_GPIO_WritePin(GPIOC, DIR1_Pin, GPIO_PIN_SET);
			tourner = 0;
			tourne_court = 0;
			}
...
{
	Vbatt = HAL_ADC_GetValue(hadc);
}
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
		if((Buffer_RX[0] != 83) && (start == 1)){
			if (!( (time_avance < 70) && (Buffer_RX[0] == AVANCE)))
				dir = Buffer_RX[0];
		}
		HAL_UART_Receive_IT(&huart3, (unsigned char *)Buffer_RX, sizeof(Buffer_RX));
